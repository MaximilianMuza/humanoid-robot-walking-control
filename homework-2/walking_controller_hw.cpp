...
void WalkingController::getZmpTrajectory()
{
  ...
  } else if(current_step_num_ >= 0 && current_step_num_ <= total_step_num_) 
  {
    ...
    if (walking_tick_ >= t_start_ && walking_tick_ < t_start_ + t_double1_) 
    {
      ...
      if (current_step_num_ > 0)
      {
        zmp_desired_(0) = d_x * (walking_tick_ - t_start_) / t_double1_ - step_length_x_ / 2;
      } else 
      {
        zmp_desired_(0) = 0;
      }
      zmp_desired_(1) = pow(-1, current_step_num_) * (d_y * (walking_tick_ - t_start_) / t_double1_ - step_length_y_ / 2);
    ...
    }  else if (walking_tick_ > t_last_ - t_double2_ && walking_tick_ <= t_last_)
    {
      ...
      zmp_desired_(1) = pow(-1, current_step_num_) * (-d_y * (walking_tick_ - t_last_ + t_double2_) / t_double2_ - step_length_y_ / 2 + d_y);
    } else if (walking_tick_ > t_last_)
    {
      zmp_desired_(0) = 0;
      zmp_desired_(1) = pow(-1, current_step_num_ + 1) * step_length_y_ / 2;
    }
  } 
